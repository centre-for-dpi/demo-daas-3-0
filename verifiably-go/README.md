...
[docs/deploy.md](docs/deploy.md#full-reset) for the exact list.
...
- **[docs/architecture.md](docs/architecture.md)** — package layout, adapter
  interface, registry routing, HTMX patterns, translation middleware, and
  the inji-proxy that bridges walt.id / Mimoto / Inji Verify quirks.

- **[docs/deploy.md](docs/deploy.md)** — deploy.sh walkthrough per scenario,
  compose overrides, seed scripts, database-repair helpers, environment
  variables, and migrating from localhost to an EC2 instance.

- **[docs/dpg-matrix.md](docs/dpg-matrix.md)** — per-DPG capability matrix,
  known upstream bugs we work around (ui-demo inji-proxy, Inji Certify kid
  mismatch, Inji Verify UI render-order config), version-compatibility
  caveats.

- **[docs/integration.md](docs/integration.md)** — adapter-to-endpoint
  mapping per DPG, how to swap `MockAdapter` for a real implementation, how
  authenticated requests flow through the OIDC providers.
