-- init-preauth.sql — derived from init.sql (sibling file in this directory).
--
-- The ONLY difference is the did_url column in the credential_config rows:
-- this file uses did:web:certify-preauth-nginx instead of did:web:certify-nginx,
-- so VCs issued by the pre-auth Inji Certify instance reference a DID that
-- resolves ONLY to the pre-auth instance's public key — independent of the
-- primary (auth-code) instance's DID document. That separation is the whole
-- reason the two instances no longer collide on kid; each signs under its
-- own DID, each has its own did.json served by its own nginx, and neither
-- can be "won" by the other during DID resolution.
--
-- Keep this file in sync with init.sql for every column OTHER than did_url;
-- if you add a credential_config row, add it to both files.
...
VALUES (
    ...
    'did:web:certify-preauth-nginx',  -- did_url (Docker-internal, HTTP resolution)
    ...
    '{"fullName": {"display": [{"name": "Full Name", "locale": "en"}]}, "mobileNumber": {"display": [{"name": "Phone", "locale": "en"}]}, "dateOfBirth": {"display": [{"name": "Date of Birth", "locale": "en"}]}, "gender": {"display": [{"name": "Gender", "locale": "en"}]}, "state": {"display": [{"name": "State", "locale": "en"}]}, "district": {"display": [{"name": "District", "locale": "en"}]}, "villageOrTown": {"display": [{"name": "Village", "locale": "en"}]}, "postalCode": {"display": [{"name": "Postal Code", "locale": "en"}]}, "landArea": {"display": [{"name": "Land Area", "locale": "en"}]}, "landOwnershipType": {"display": [{"name": "Ownership", "locale": "en"}]}, "primaryCropType": {"display": [{"name": "Primary Crop", "locale": "en"}]}, "secondaryCropType": {"display": [{"name": "Secondary Crop", "locale": "en"}]}, "farmerID": {"display": [{"name": "Farmer ID", "locale": "en"}]}}'::JSONB,  -- credential_subject
...
-- ---------------------------------------------------------------------------
-- FarmerCredentialV2 — W3C VCDM v2.0 (validFrom/validUntil, credentials/v2)
-- ---------------------------------------------------------------------------
INSERT INTO certify.credential_config (
    ...
    'active',
    '',
    NULL,  -- doctype
    NULL,  -- sd_jwt_vct
    ...
    NULL,
    '[{"name": "Farmer Credential (V2)", "locale": "en", "logo": {"url": "https://mosip.github.io/inji-config/logos/agro-vertias-logo.png", "alt_text": "Farmer Credential Logo"}, "background_color": "#1a5c2e", "text_color": "#FFFFFF"}]'::JSONB,
    ...
    'mock_identity_vc_ldp',  -- same scope as v1 (eSignet only has this scope registered)
    ARRAY['did:jwk'],
    ...
    ARRAY['revocation'],
    '[{"Full Name": "${fullName}", "Date Of Birth": "${dateOfBirth}"}]'::JSONB,
    'EdDSA',
    NOW(),
    NULL
);
...
-- ---------------------------------------------------------------------------
-- FarmerCredentialSdJwt — SD-JWT VC (vc+sd-jwt, selective disclosure)
-- ---------------------------------------------------------------------------
INSERT INTO certify.credential_config (
    credential_config_key_id, config_id, status, vc_template, doctype, sd_jwt_vct,
    context, credential_type, credential_format, did_url,
    ...
    sd_claim, display, display_order, scope,
    cryptographic_binding_methods_supported, credential_signing_alg_values_supported,
    proof_types_supported, credential_subject, sd_jwt_claims, mso_mdoc_claims,
    plugin_configurations, credential_status_purpose, qr_settings, qr_signature_algo,
    cr_dtimes, upd_dtimes
) VALUES (
    'FarmerCredentialSdJwt',
    gen_random_uuid()::VARCHAR(255),
    'active',
    '',
    NULL,  -- doctype
    'https://example.com/credentials/FarmerCredential',  -- sd_jwt_vct
    NULL,  -- context (not used for SD-JWT)
    NULL,  -- credential_type (not used for SD-JWT)
    'vc+sd-jwt',
    'did:web:certify-preauth-nginx',
    'CERTIFY_VC_SIGN_ED25519',
    'ED25519_SIGN',
    'EdDSA',
    'Ed25519Signature2020',
    NULL,  -- sd_claim: NULL means no selective disclosure (all claims visible)
    '[{"name": "Farmer Credential (SD-JWT)", "locale": "en", "logo": {"url": "https://mosip.github.io/inji-config/logos/agro-vertias-logo.png", "alt_text": "Farmer Credential Logo"}, "background_color": "#8B4513", "text_color": "#FFFFFF"}]'::JSONB,
    ARRAY['fullName', 'mobileNumber', 'dateOfBirth', 'gender', 'farmerID'],
    'mock_identity_vc_ldp',  -- same scope (eSignet only has this scope registered)
    ARRAY['did:jwk'],
    ARRAY['Ed25519Signature2020'],
    '{"jwt": {"proof_signing_alg_values_supported": ["RS256", "ES256"]}}'::JSONB,
...
CREATE TABLE certify.status_list_credential (
    ...
    capacity_in_kb BIGINT,                  --- length of status list (in KB)
