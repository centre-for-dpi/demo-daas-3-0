...
// client. Used by the inji.walletStore to claim credentials from Inji Certify
// without requiring a separate wallet implementation.
//
// The client:
...
// ClaimInjiCredential runs the full OID4VCI pre-auth claim flow for a single
// offer URL and returns the raw credential JSON that the issuer returned.
//
// This is intentionally self-contained: no external JWT libraries, stdlib only.
func ClaimInjiCredential(ctx context.Context, offerURL string) ([]byte, error) {
...
// rewriteForHost maps Inji's configured internal URL (certify-nginx:80) to the
// host-accessible URL (localhost:8091). This is necessary because our Go server
// runs on the host, not inside the waltid_default docker network.
